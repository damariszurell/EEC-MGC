...
Thus, ensembles can be easily constructed by hand. Of course, if you have many input data, models, and scenarios to put into your ensemble, this is easily becoming tedious. For that purpose, it will be very useful to automatise your code and write automatic functions. For an example for automatising your functions, have a look at the code that we published along with @Zurell2019, which you can obtain from the corresponding [github repository](https://github.com/damariszurell/SSDM-JSDM). Also, there are packages like `biomod2` that make it easy to construct ensembles automatically [@Thuiller2009].
...
(perf_mean_prob <- evalSDM(sp_test$Turdus_torquatus, mean_prob))

# performance measures for "median of probabilities":
(perf_median_prob <- evalSDM(sp_test$Turdus_torquatus, median_prob))

# performance measures for "weighted mean of probabilities":
(perf_wmean_prob <- evalSDM(sp_test$Turdus_torquatus, wmean_prob))

# performance measures for "committee average":
(perf_committee_prob <- evalSDM(sp_test$Turdus_torquatus, committee_av))
...
Remember how we analysed biodiversity changes in [practical a6](a6_BiodivChanges.html).

- How much are your species predicted to win or lose in range size under climate change?
